...
<div class="fieldcontain ${hasErrors(bean: cropInstance, field: 'pesticide', 'error')} ">
    <label for="pesticide">
        <g:message code="crop.pesticide.label" default="Pesticide"/>
...
%{--<g:if test="${session.getAttribute("userId")}" >
                <div class="fieldcontain ${hasErrors(bean: cropInstance, field: 'user', 'error')} required">
                    ...
                    <g:select id="user" name="user.id" from="${nasacropalert.User.list()}" optionKey="id" required="false"
                              value="${cropInstance?.user?.id}" class="many-to-one"/>
                </div>
</g:if>
<g:else>
